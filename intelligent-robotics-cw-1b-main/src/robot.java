 ...
 */
    	...
    	double p1 = (double) px*Math.cos(theta1) + py*Math.sin(theta1) - d4*Math.cos(90-alpha);
    	double p2 = pz - d4*Math.sin(90-alpha);
    	
    	double c3 = (sqr((float)p1) + sqr((float)p2) - sqr(d2) - sqr(d3)) / (2*d2*d3);
    	
    	 theta3 = (float) Math.atan2(Math.sqrt(1-sqr((float)c3)), c3);
    	
    	 theta2 =  (float) Math.atan2(((d3*c3+d2)*p2-d3*Math.sin(theta3)*p1), ((d3*c3+d2)*p1+d3*Math.sin(theta3)*p2));
    	
    	 theta4 = (90-alpha) -theta2 - theta3;
